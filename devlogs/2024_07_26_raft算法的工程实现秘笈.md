...
1. “快速index匹配算法”一节，如果发生了分叉，tryFindMatchPos不能通过leader/follower各调用一次就找到正确的位点，但是重复这个过程最终会收敛。
2. “快速index匹配算法”一节，e部分，leader调用tryFindMatchPos(4,7)返回值应该为：term=4，**index=5**，e和f都需要两次匹配
3. “raft成员变更”一节，“候选人发起投票前，应该先执行完它已有的日志”不严谨，应该改为“候选人发起投票前，应该先执行完已commit的日志”