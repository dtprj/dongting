...
- [WorkerStatus.java](file://client/src/main/java/com/github/dtprj/dongting/net/WorkerStatus.java) - *客户端网络层对象分配优化*
- [RefBuffer.java](file://client/src/main/java/com/github/dtprj/dongting/buf/RefBuffer.java) - *客户端网络层对象分配优化*
- [RaftInput.java](file://server/src/main/java/com/github/dtprj/dongting/raft/server/RaftInput.java) - *服务器端RAFT流控优化*
- [TailCache.java](file://server/src/main/java/com/github/dtprj/dongting/raft/impl/TailCache.java) - *服务器端RAFT流控优化*
</cite>
...
## 引言
本文档旨在为Dongting项目提供全面的性能优化指南。重点介绍对象池（ByteBufferPool）的设计原理和使用方法，解释其如何通过复用ByteBuffer来减少垃圾回收（GC）的压力，特别是在处理大容量缓冲区时。同时，文档将介绍项目中使用的其他性能优化技术，如高效的序列化（PbUtil）和无锁数据结构，并指导用户如何使用benchmark模块进行性能基准测试。最后，文档将分享来自devlogs的性能优化经验和最佳实践。
...
在客户端网络层中，通过`RefBuffer`和`WorkerStatus`实现了高效的对象分配与管理。`RefBuffer`继承自`RefCount`，用于对`ByteBuffer`进行引用计数管理，确保在多线程环境下能够安全地共享和释放缓冲区。

**RefBuffer实现细节**

`RefBuffer`在构造时根据请求大小和阈值决定是否从对象池中借用缓冲区。对于小于阈值的缓冲区直接分配，而对于大于阈值的缓冲区则从对象池中借用，从而减少大缓冲区的频繁创建与销毁。
...
`WorkerStatus`类负责管理网络工作线程的状态，包括待写入的数据包数量、待释放的缓冲区计数和字节数。通过`toReleaseCount`和`toReleaseBytes`字段跟踪需要释放的资源，确保在网络操作完成后及时回收内存。
...
**代码来源**
- [WorkerStatus.java](file://client/src/main/java/com/github/dtprj/dongting/net/WorkerStatus.java#L56-L57)

### 服务器端RAFT流控优化

在服务器端，RAFT协议的流控机制通过`RaftInput`和`TailCache`实现，有效控制了日志条目的缓存和处理，避免内存溢出并提升系统稳定性。

**RaftInput中的流控大小计算**

`RaftInput`类在构造时计算输入数据的流控大小，该大小用于后续的流控决策。流控大小包括头部和主体的实际大小之和，确保系统能够根据数据量进行合理的资源分配。
...
**代码来源**
- [RaftInput.java](file://server/src/main/java/com/github/dtprj/dongting/raft/server/RaftInput.java#L48-L55)

**TailCache中的流控管理**

`TailCache`类维护了一个缓存队列，用于存储待处理的RAFT任务。通过`pendingCount`和`pendingBytes`字段跟踪待处理任务的数量和总字节数，并在达到配置上限时触发清理操作，防止内存无限增长。
...
**代码来源**
- [TailCache.java](file://server/src/main/java/com/github/dtprj/dongting/raft/impl/TailCache.java#L73-L82)
