...
public class InstallTest extends ServerTestBase {
    ...
    private void testNewEmptyFollowerAddToGroup(int count, int bodySize) throws Exception {
        ...
        TestUtil.waitUtil(() -> g3.groupComponents.raftStatus.getShareStatus().lastApplied >= lastRaftIndex.get());

        // put after install
        String lastKey = "lastKey";
        byte[] lastBody = new byte[bodySize];
        r.nextBytes(lastBody);
        long raftIndex = client.put(groupId, lastKey.getBytes(), lastBody);
        expectMap.put(lastKey, lastBody);
        TestUtil.waitUtil(() -> g3.groupComponents.raftStatus.getShareStatus().lastApplied >= raftIndex);
        ...
        actualMap.clear();
        list = client.batchGet(groupId, keys);
        for (int i = 0; i < list.size(); i++) {
            actualMap.put(new String(keys.get(i)), list.get(i).getData());
        }
        for (String k : expectMap.keySet()) {
