...
```java
// dongting 支持multi-raft，因此需要指定group id，默认group id是1
int groupId = 1;
...
// 在运行时添加group定义，这里添加一个 groupId 为 1 的组，包含 id 为 1、2、3 的 3 个节点
...
5. 修改servers.properties文件中的`group.1.nodeIdOfMembers`参数，3节点的话可能是`1,2,3`
...
假设你有一个3节点的集群，节点ID为1、2、3，group ID为1，现在需要删除节点3。操作步骤如下：
...
3. 提交配置变更：
   ```sh
   ./dongting-admin.sh commit-change --group-id 1 --prepare-index <返回的prepare-index>
   ```
...
假设你有一个3节点的集群，节点ID为1、2、3，group ID为1，现在需要添加节点4。操作步骤如下：
